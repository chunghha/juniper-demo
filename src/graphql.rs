...
impl QueryRoot {
  async fn customer(ctx: &Context, id: String) -> juniper::FieldResult<Customer> {
    ...
    let row =
      ctx.db_client.query_one("SELECT name, age, email, address FROM customers WHERE id = $1", &[&uuid]).await?;
    let customer =
      Customer { id, name: row.try_get(0)?, age: row.try_get(1)?, email: row.try_get(2)?, address: row.try_get(3)? };
    info!("Customer {:#?} searched by {:#?}.", &customer.email, &uuid);
    Ok(customer)
  }

  async fn customers(ctx: &Context) -> juniper::FieldResult<Vec<Customer>> {
    let rows = ctx.db_client.query("SELECT id, name, age, email, address FROM customers", &[]).await?;
...
impl MutationRoot {
  ...
  async fn update_customer_email(ctx: &Context, id: String, email: String) -> juniper::FieldResult<String> {
    let uuid = uuid::Uuid::parse_str(&id)?;
    let email = email.to_lowercase();
    let n = ctx.db_client.execute("UPDATE customers SET email = $1 WHERE id = $2", &[&email, &uuid]).await?;
    if n == 0 {
  ...
  async fn delete_customer(ctx: &Context, id: String) -> juniper::FieldResult<bool> {
    let uuid = uuid::Uuid::parse_str(&id)?;
    let n = ctx.db_client.execute("DELETE FROM customers WHERE id = $1", &[&uuid]).await?;
    if n == 0 {
      return Err("User does not exist".into());
    }
    info!("{:#?} deleted.", &id);
